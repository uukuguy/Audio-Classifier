...
def main():
    ...
    for gi, cid in enumerate(conv):
        ...
        for e in range(CTX, len(a) - TGT + 1, args.stride):
            ...
            repr256 = out["x"][:, -POOL:, :].mean(1).squeeze(0).cpu().numpy()  # 256d 冻结表征
            feats.append(repr256)
            ctxs.append(ctxfeat(a[e - CTX:e]))
            fut = set(int(x) for x in a[e:e + TGT])
            y.append(1 if BC in fut else 0)
            G.append(gi)
        if (gi + 1) % 20 == 0:
            print(f"[kernel] {gi+1}/{len(conv)} convs, {len(feats)} samples", file=sys.stderr)
    ...
    for name, mk in models.items():
        try:
            o = oof(mk, use_proba=True)
            auc = roc_auc_score(y, o)
            bf = max(f1_score(y, (o >= t).astype(int), zero_division=0)
                     for t in np.linspace(0.05, 0.95, 19))
            res[name] = {"bc_f1": round(bf, 4), "auc": round(auc, 4)}
