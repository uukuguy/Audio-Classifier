...
class VAPTurnTaking(nn.Module):
    """v2 (2026-05-30): 修正 v1 弱构型 (用户质疑音频证伪过快).
    v1 问题: mean-pool 末M帧把 50Hz 时序坍缩成1点 → 抹掉 BC 韵律动态.
    v2: attention-pool over 末窗帧序列 (模型自学时序权重, 保韵律/onset/停顿).
    """
    def __init__(self, vap: nn.Module, ctx_dim: int, vap_dim: int = 256,
                 pool_frames: int = 50):  # v2: 末50帧=1s 时序 (v1只10帧=200ms)
        super().__init__()
        self.vap = vap
        self.pool_frames = pool_frames
        self.cn = nn.LayerNorm(ctx_dim)
        # attention pooling: 学习给末窗每帧的权重 (替代 mean — 保时序动态)
        self.attn_q = nn.Parameter(torch.randn(vap_dim) * 0.02)
        self.attn_proj = nn.Linear(vap_dim, vap_dim)
        self.audio_ln = nn.LayerNorm(vap_dim)
        fin = ctx_dim + vap_dim
    ...
    def forward(self, waveform: torch.Tensor, ctx: torch.Tensor) -> torch.Tensor:
        ...
        x = out["x"][:, -self.pool_frames:, :]            # [B, M, 256] 末窗帧序列(保时序)
        # attention pool: 每帧打分 → softmax → 加权和 (模型自学哪帧重要)
        h = torch.tanh(self.attn_proj(x))                 # [B, M, 256]
        scores = (h * self.attn_q).sum(-1)                # [B, M]
        w = torch.softmax(scores, dim=1).unsqueeze(-1)    # [B, M, 1]
        audio_feat = self.audio_ln((x * w).sum(dim=1))    # [B, 256] 加权时序池化
        c = self.cn(ctx)
