...
### D-29: S5 是当前素材局部峰 — 单源软加叠加路线终结 (押风险博 #1 失败)

**触发**: 6/7 day9 push-1 A/B 真分回完, 押"在 S5 上加第三个 src 软加博 +0.008" 失败.

**证据 (在 S5=0.747131 上软加新 src)**:
| push | 配方 | 权重 | 真分 | Δ vs S5 |
|---|---|---|---|---|
| P1 (6/6) | S5 + wsp_ms | 0.10 | 0.741037 | -0.006 |
| P3 (6/6) | S5 + e2v_ms | 0.05 | 0.736542 | -0.011 |
| P5 (6/6) | R4 + omni7b_ms2 | 0.05 | 0.747569 | +0.0004 (噪声) |
| A (6/7) | S5 + omni7b_ms2 | 0.03 | 0.746166 | -0.001 (噪声) |
| B (6/7) | S5 + qwen17b_ms2 | 0.03 | 0.741489 | -0.0056 |

**结论**: 在 S5/R4 上**线性叠加任何"再多一个 src"**都跌. 不论 src 类型 (SSL 多源 wsp_ms/e2v_ms/hub_ms 已在 R4 内 / 多模态 LLM omni7b / 文本 LLM qwen17b), 不论权重 (0.03/0.05/0.10). **现有素材池靠"再加一源软加"撞不到 #1 (距 +0.0076)**.

**根因**: R4 + omni3b 0.05 配方已饱和当前素材池的有效信号维度. 新源即使在 OOF 上看似正交, 实际在公榜测试集 1 分布上跟 R4 内部源 redundant.

**接下来 (回 D-27 战略, 强化)**:
- ❌ 现有素材"再加一个 src 软加"路线**关闭**, 不再单 push 验证 (除非有新训练出的源)
- ✅ **真要冲 #1 必须改 R4 内部某个源** (换更强 hub_ms / e2v_ms / 换 ctx base 编码方式), 而**不是线性叠加**
- ✅ 接受 #2/#3 公榜位置, 复赛准备压倒 (D-27 重申, 6/16 截止剩 9 天)
