...
> Generated by `tools/climb/regen-tree.py` (deterministic — 25 runs logged)
> Do NOT edit — re-generated on every push / LB landed / cycle complete.

**SOTA**: probe-5push-cand2-omni020-20260602-1412 = 0.73 (paradigm omni-lora-softadd)

## In-flight / session state (dynamic — resume reads this, then verifies liveness)

- **phase**: D-22 范式反转后. NEW SOTA 0.72852 (Omni-7B 软加 0.2, 9.4B 超 8B). 合规 SOTA 0.71755 (3src). 冲前 20 (门槛 0.7256), 前 10 (~0.748). 剩 8 天 × 5 push = 40 push 配额. 6/10 前定基础模型.
- **best online**: 0.72852
- **last_cycle**: 24
- **next_hypothesis**: ?

**In-flight job** (resume MUST verify liveness):
- task: 无 (Omni-3B 23:24 完成, heartbeat 自动退出. 5 push 候选全就位)
- next_action: 用户 6/3 提交 5 push 后贴真分回来, 跑 tools/climb/calibrate_push_results.py --scores S1 S2 S3 S4 S5
...
graph LR
    ...
    class probe_5push_cand2_omni020_20260602_1412 sota
