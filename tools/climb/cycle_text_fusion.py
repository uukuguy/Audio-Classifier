"""climb cycle — paradigm=text-lexical-fusion (H-T1, 测 ASR 词汇能否救 BC).
...
def tune_threshold(y, p):
    """温和阈值：只在 [0.35,0.65] 搜（铁律：滑窗CV调激进阈值线上更差）。"""
    bt, bf = 0.5, -1.0
...
def main():
    ...
    (run_dir / "cv_metrics.json").write_text(json.dumps({
        "paradigm": "text-lexical-fusion", "hypothesis_id": "H-T1", "cv_macro_f1": round(macro, 4),
    ...
    (run_dir / "manifest.json").write_text(json.dumps({
        "cycle": 4, "hypothesis_id": "H-T1", "paradigm": "text-lexical-fusion",
