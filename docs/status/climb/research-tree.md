...
> Generated by `tools/climb/regen-tree.py` (deterministic — 46 runs logged)
...
- **phase**: 复赛端到端镜像阶段. cycle45 H-F1完成: 重写干净三源架构(ctx+wsp+hub orthofuse-3src) + 构建team26:r3-base-20260624(20.4G) + 镜像内冒烟通过 + docker push. 融合逻辑identical初赛0.71755. 等官方真分校准.
- **best online**: 0.747569
- **last_cycle**: 45
- **next_hypothesis**: ?

**In-flight job** (resume MUST verify liveness):
- task: docker push team26:r3-base-20260624 (复赛首个干净端到端镜像). pending-lb 1条.
- next_action: push完成→给用户tag/url提交提示. 用户提交url到官方后台→评测自动评分→贴回真分. 关注GPU耗时<60min + 单seed vs 3seed差异. 下一cycle候选: H-F5阈值精调/H-F1b e2v还原/H-F4多seed
...
| S5-keep125-10s-20260607 | cross-context-probe | S5-R4+omni3b005-20260605 | — | 0.72 | PUSH | 🟢 confirmed cross-ctx degradation |
| S5-keep63-5s-20260607 | cross-context-probe | S5-R4+omni3b005-20260605 | — | 0.71 | PUSH | 🟢 confirmed cross-ctx degradation |
| S5-keep250-20s-20260607 | cross-context-probe | S5-R4+omni3b005-20260605 | — | 0.72 | — | 🟢 confirmed -0.030 vs 30s, anomalously below 10s (0.7225) |
| hf1-smoke5-20260624 | finals-e2e-core4 | finals-e2e-core4 | — | — | NO-PUSH | 🟡 diagnosis-only |
| r3-base-20260624 | finals-e2e-core4-orthofuse | finals-orthofuse-3src | — | — | PUSH | 🟡 pending-lb |
...
graph LR
    ...
    S5_keep125_10s_20260607["S5-keep125-10s-20260607<br/>0.72"]
    class S5_keep125_10s_20260607 confirmed
    S5_keep63_5s_20260607["S5-keep63-5s-20260607<br/>0.71"]
    class S5_keep63_5s_20260607 confirmed
    S5_keep250_20s_20260607["S5-keep250-20s-20260607<br/>0.72"]
    class S5_keep250_20s_20260607 confirmed
    hf1_smoke5_20260624["hf1-smoke5-20260624<br/>—"]
    r3_base_20260624["r3-base-20260624<br/>—"]
    class r3_base_20260624 pending
    ...
    S5_R4_omni3b005_20260605 --> S5_keep250_20s_20260607
    finals_e2e_core4 --> hf1_smoke5_20260624
    finals_orthofuse_3src --> r3_base_20260624
```

## Hypothesis pool

### Active (27) — ranked by priority
...
- **H-F2** (rank 0.90, cost 1h, pending): 镜像工程修复: pynvml + Dockerfile唯一源 + 瘦身COPY + tag规范 + 删infer_entry.sh冗余
  - lift: 0 分变化, 但解决官方硬要求(pynvml)+ 可维护性 + 镜像瘦身(去tools/cloud)
...
- **H-F3** (rank 0.85, cost 1h, pending): 变长(5-30s)端到端正确性验证: featurize_variable_length + normalize_ctx_to_375 在不定长ctx上跑通
  - lift: 保证私有测试集2(5-30s不定长)不崩 + 不退化(初赛D-26~D-31已研变长退化曲线)
...
- **H-F5** (rank 0.75, cost 0.5h, pending): 阈值还原: 当前THR硬编码偏离初赛thresholds.json (NA 0.05→0.25, BC 0.5→0.75, I 0.5→0.65)
  - lift: +? NA是第二大类(30%), 阈值0.05→0.25大量砍NA正例→NA F1崩 (违阈值铁律)
...
- **H-F4** (rank 0.70, cost 1.5h, pending): per-class正交融合还原初赛orthofuse配方 (当前fuse_s5是硬编码简化加权)
  - lift: +? 对齐初赛真SOTA融合 (orthofuse per-class跨源正交 vs 当前简单加权)
...
- **H-F1b** (rank 0.50, cost 2h, pending): e2v源还原: WavLMModel→funasr.AutoModel加载 + Dockerfile装funasr, 验证e2v真实增益值不值funasr重依赖
  - lift: e2v初赛融合权重0.03(边际源). 修复后验证端到端真增益; 若<noise则砍, 三源即主配方
...
- **H-D22-10** (rank 0.40, cost 1.5h, pending): TTA (test-time augmentation): test 音频 / 增强变体多次推理平均
  - lift: +0.002~0.005
- **H-F6** (rank 0.40, cost 3h, pending): (条件性,基线后)按ROI加源: Omni-3B / qwen2audio / wavlm / qwen3文本头
  - lift: Omni +0.0004(微+占8B大半) / 新异质源未知, 需查初赛离线融合贡献
...
### Pending LB (1)
- r3-base-20260624 (pushed 2026-06-24T08:35:00+08:00, paradigm finals-e2e-core4-orthofuse)
